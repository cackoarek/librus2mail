...
- **Entrypoint**: `main.py` (infinite loop checking messages and announcements).

## Agent Directives

1. **Security & Secrets**:
   - Strictly avoid staging or committing `config.yaml`, `arek_config.yaml`, `*_config.yaml`, `.env`, or `*.log`.
   - Never run `python main.py` autonomously; it interacts with live school accounts and sends real emails.
