...
class FileStorage(BaseStorage):
    ...
    def save_known_items(self, user_login: str, known_messages: set, known_notifications: set, known_grades: set) -> None:
        ...
        data = {
            'librus_login': str(user_login),
            'last_updated': datetime.now().isoformat(),
            'known_messages': sorted(list(known_messages)),
            'known_notifications': sorted(list(known_notifications)),
            'known_grades': sorted(list(known_grades))
        }
        try:
